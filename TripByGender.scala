...
object TripByGender {
  def main(args: Array[String]) {
        val sparkConf = new SparkConf().setMaster("local[2]").setAppName("TripByGender")
        val scc = new StreamingContext(sparkConf, Duration(30000))
      //  scc.checkpoint(".") // 因为使用到了updateStateByKey,所以必须要设置checkpoint
        val topics = Set("tripdata") //我们需要消费的kafka数据的topic
        val kafkaParam = Map(
        ...
        val tmp = stream.map(_._2)      // 取出value
                  .map(rec => rec.split(","))
                  .map(rec => (rec(14),rec(0).toInt))
                  .reduceByKey(_+_)
                  .transform(rec => rec.sortByKey(ascending = false))
                  
          
        tmp.print() // 打印前10个数据
        tmp.saveAsTextFiles("/home/hadoop/test/TripByGender")   
        scc.start() // 真正启动程序
