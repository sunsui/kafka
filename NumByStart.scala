...
object NumByStart {
  def main(args: Array[String]) {
        val sparkConf = new SparkConf().setMaster("local[2]").setAppName("NumByStart")
        val scc = new StreamingContext(sparkConf, Duration(60000))
      //  scc.checkpoint(".") // 因为使用到了updateStateByKey,所以必须要设置checkpoint
        ...
        tmp.print() // 打印前10个数据
        tmp.saveAsTextFiles("/home/hadoop/test/NumByStart")   
        scc.start() // 真正启动程序
